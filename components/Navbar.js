...
const Navbar = ({ logout, user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
    ...
    return (
        <>
            <span>
                { Dropdown && <div onMouseOver={ () => { setDropdown(true) } } onMouseLeave={ () => { setDropdown(false) } } className='fixed md:top-10 top-9 py-3 right-14 rounded-md px-5 bg-white shadow-lg border w-40 z-30'>
                    <ul>
                        <Link href={ '/myaccount' }><a><li className='py-1 hover:text-pink-700 text-sm font-semibold'>My Account</li></a></Link>
                        <Link href={ '/orders' }><a><li className='py-1 hover:text-pink-700 text-sm font-semibold'>My Orders</li></a></Link>
            ...
            <div className={ `flex flex-col justify-center items-center md:flex-row md:justify-start py-1 shadow-md sticky top-0 z-10 bg-white ${!sidebar && "overflow-hidden"}` }>
                ...
                <div ref={ ref } className={ `z-50 w-72 h-[100vh] sideCart overflow-y-scroll absolute top-0 bg-pink-100 py-10 px-7 transition-all ${sidebar ? 'right-0' : '-right-96'}` }>
                    ...
                    <div className='flex justify-between mt-5'>
                        <Link href={ '/checkout' }><a><button disabled={ Object.keys(cart).length === 0 } className="disabled:bg-pink-300 flex text-white bg-pink-500 border-0 py-2 px-5 focus:outline-none hover:bg-pink-600 rounded text-sm">  Checkout </button></a></Link>
                        <button disabled={ Object.keys(cart).length === 0 } onClick={ clearCart } className=" disabled:bg-pink-300 flex text-white bg-pink-500 border-0 py-2 px-5 focus:outline-none hover:bg-pink-600 rounded text-sm">Clear Cart</button>
